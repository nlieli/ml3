...
# custom classes
from models import Outlier_Detector, Base_Model, RF_Model, SVM_Model, XGB_Model, PIP_Model

# model configs
from model_configs import rf_models_config, svm_models_config, xgb_models_config

def predict(X_test, model: RF_Model):
...
def generate_submission(test_data, model: RF_Model):
    label_predictions, outlier_predictions = predict(test_data, model)
...
def main():
    ...
    submission_df = generate_submission(df_leaderboard, rf_model)
    ...
    submission_df = generate_submission(df_final, rf_model)
